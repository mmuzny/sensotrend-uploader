  ...
  */
  ...
  var setDateTime = function(serverTime, cb) {
    ...
    async.series({
      ...
      eot: function(callback) {
        if (cfg.deviceInfo.model !== 'Contour Next One' || cfg.deviceInfo.model !== 'Contour Next One') {
  ...
  return {
      ...
      async.whilst(
        ...
        function (err) {
          ...
          if(err || error) {
            ...
            if (cfg.deviceInfo.driverId === 'ContourPlusOne') {
    ...
    uploadData: function (progress, data, cb) {
      ...
      const timeSort = (a,b) => a.time < b.time ? -1 : 1;
      const singleRecord = [data.post_records.sort(timeSort).pop()];
      debug('Uploading a single record', data.post_records, singleRecord);
      progress(100);
      return cb(null, data);

      /*
      cfg.api.upload.toPlatform(singleRecord, sessionInfo, progress, cfg.groupId, function (err, result) {
        progress(100);
